...
Finally, recent analysis \cite{Ekera2021} has proven that once the order of a randomly chosen element of $(\mathbb{Z}/N\mathbb{Z})^*$ has been obtained, regardless of whether the order is odd or even, N can be recovered with very high probability\footnote{The only points of concern are trivial values for $r$, but the probability of finding one of these tends to 0 as N tends to infinity, so for large values this probability is extremely low.} using classical post-processing. Odd orders affect the runtime,but the asymptotic complexity remains the same.
