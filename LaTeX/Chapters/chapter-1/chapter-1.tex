...
\label{eq2.1}
\end{equation}
for some $r$ with $1<r<\Phi(N)$\footnote{$r$ is bounded up by $\Phi(N)$ due to Lagrange's theorem}, where r is called the multiplicative order, or \textbf{period}, of $a$ modulo $N$ and $\Phi(N)$ is Euler's totient function. It should be noted that $r$ is defined as the smallest possible number that satisfies \eqref{eq2.1}.
...
From \eqref{eq2.2}, a few things can be seen. First, our logic from now on will work only for even $r$. In the case that $r$ happens to be odd, Shor picks a new $a$, computes a new $r$,and relies on probabilistically finding an even $r$.
