...
a^r \equiv 1 \pmod N
\end{equation}
for some $r$ with $1<r<\Phi(N)$\footnote{$r$ is bounded up by $\Phi(N)$ due to Lagrange's theorem}, where r is called the multiplicative order, or \textbf{period}, of $a$ modulo $N$ and $\Phi(N)$ is Euler's totient function. It should be noted that $r$ is defined to be the smallest possible number that satisfies \textbf{Eq 2.1}.
