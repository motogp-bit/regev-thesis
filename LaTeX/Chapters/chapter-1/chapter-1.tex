...
\label{eq2.1}
\end{equation}
for some $r$ with $1<r<\Phi(N)$\footnote{$r$ is bounded up by $\Phi(N)$ due to Lagrange's theorem}, where r is called the multiplicative order, or \textbf{period}, of $a$ modulo $N$ and $\Phi(N)$ is Euler's totient function. It should be noted that $r$ is defined to be the smallest possible number that satisfies \eqref{eq2.1}.
...
From \eqref{eq2.1}, it is obvious that  $N\mid(a^r - 1)$. This implies (but is not equivalent to):
