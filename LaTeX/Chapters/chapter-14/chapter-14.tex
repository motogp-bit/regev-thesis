...
To explore this concept, first we need to talk about Zeckendorf's theorem. The statement is as follows:\\
...
\begin{equation}
    ...
    \label{eq15.2}
\end{equation}
where $c_{i,j}$ denotes each of the earlier $c_j$ but now for every base. Then we can plug \eqref{eq15.2} in the place of each exponent and write the resulting expression - which is \textbf{equivalent} to the Regev product, as follows: $$\prod_{j=1}^m(\prod_{i=1}^d b_i^{c_{i,j}})^ {F_j}$$
...
Notice how the Fibonacci sequence appears in the exponents! Now we have a reversible base accessing operation that does not leave us with dirty qubits in every iteration. More importantly, it seems that this can actually decrease the qubit count using the optimized multi-exponentiation idea from Chapter 7 with the reduced register sizes, because we will not need the extra $O(n^{3/2})$ qubits to perform squaring anymore.

Let us, finally, describe Algorithm 5.2 from Ragavan and Vaikuntanathan\cite{RagavanVaikuntanathan2023} in terms of what we wrote earlier. It is important to note that this is applied to the shifted exponent $e_i = c_i + D/2$. In order to shorten the notation, we will denote by $*$ the act of applying the corresponding operation to the inverse.

\begin{algorithm}[H]
\caption{Fibonacci multi-exponentiation (top-down)}
\textbf{Input:} Initial quantum state $\ket{e}, m$\\
\textbf{Output:} State consisting of a superposition $\prod_{i=1}^d b_i^{e_i} \pmod N$
\begin{algorithmic}[1]
\State Compute $c_{i,j}$ in superposition in accordance with our base algorithm.
\State Initialize registers $\ket{x_1}\ket{x_1^{-1}} \gets (1,1)$ and $\ket{x_2}\ket{x_2^{-1}} \gets (1,1)$
\For {$j = m,m-1 \dots$ down to 1}
    \State Compute the tuple $\ket{C_j}\ket{C_j^{-1}}$ for the current step in the manner we described in Chapter 13, entangling the $e_i$ in a superposition.
    \State $\ket{x_1}\ket{x_2} \gets \ket{x_1 \cdot x_2 \pmod N},\ket{(x_1 \cdot x_2)^{-1} \pmod N}$,
    \State $\ket{x_2}\ket{x_2^{-1}} \gets \ket{x_2 \cdot C_j \pmod N},\ket{(x_2 \cdot C_j)^{-1} \pmod N}$
    \State Swap $\ket{x_1}\ket{x_1^{-1}}$ and $\ket{x_2}\ket{x_2^{-1}}$
    \State Run the inverse of the circuit used to construct $\ket{C_j}\ket{C_j^{-1}}$ to uncompute it into $\ket{0} \ket{0}$
\EndFor
...
We should mention that the classical postprocessing differs a bit, specifically the part we outlined in Chapter 10. With respect to complexity, given a circuit that implements something similar to $\eqref{eq13.2}$ with $S = n+ O(1)$ qubits and $G$ gates, the above algorithm can be executed with $S + (A/log\phi + 6 + O(1))n$ qubits and $O(\sqrt{n}G + n^{3/2})$ gates assuming an arbitrary multiplication circuit. Assuming we adapt the classical Van Der Hoeven algorithm for multiplication\cite{HarveyVanDerHoeven2021}, this is equivalent to $O(nlogn)$ qubits and $O(n^{3/2} logn)$ gates, achieving a significant improvement over Regev's $O(n^{3/2})$ qubits.
