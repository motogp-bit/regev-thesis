...
\\
 ...
 Consider that a finite n-dimensional lattice modulo N whose elements are vectors that satisfy the congruence in \eqref{eq5.4} is a subgroup of the group of all vectors with integer coordinates mod N (denoted $Z_N^n$), so we can define a coset of that lattice with respect to that group accordingly. Crucially, replacing 1 with $u$ for some arbitrary $u \in \mathbb{Z}_N$ defines a coset whose representative is some exponent vector $\mathbb{t}$ that represents the exact shift that must be applied to the $v_i$ to get $u$ instead of 1. This will become important later.
