...
forms a subgroup of $\mathbb{Z}^d$ under vector addition, and therefore a lattice.\footnote{\eqref{eq5.4} is a linear congruence in disguise. To see that, consider what happens when taking the logarithm of both sides.} 
...
Therefore, the set of all vectors whose coordinates satisfy \eqref{eq5.4} forms a finite lattice $\Lambda$, and our proof is complete.
 ...
 Consider that a finite n-dimensional lattice modulo N whose elements are vectors that satisfy the congruence in \eqref{eq5.4} is a subgroup of the group of all vectors with integer coordinates mod N (denoted $Z_N^n$), so we can define a coset of that lattice with respect to that group accordingly. Crucially, replacing 1 with $u$ for some arbitrary $u \in \mathbb{Z}_N$ defines a coset whose representative is some exponent vector $\mathbb{t}$ that represents the exact shift that needs to be applied to the $v_i$ to get $u$ instead of 1. This will come into importance later.
