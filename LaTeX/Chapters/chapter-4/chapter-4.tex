...
\textbf{Definition 5.1 (Lattice)} A lattice is a discrete additive subgroup of $\mathbb{R}^n$, i.e. it is a subset $\Lambda \subseteq \mathbb{R}^n$ satisfying the following properties:
...
\begin{equation}
    \Lambda^n = \set{z_1b_1+z_2b_2+\dots +z_nb_n | z_i \in \mathbb{Z},b_i \in \mathbb{R}^n}
\end{equation}
...
Now, given a lattice $\Lambda$, the \textbf{Shortest Vector Problem} (SVP) asks to find the non-zero vector with minimal Euclidean length. That is, we are looking for $v_0$, such that:
\begin{equation}
...
Given a function $f: \mathbb{Z}\to S$ that is \textbf{periodic}, meaning that there exists some $r > 0$ s.t. $f(x+r) = f(x)\: \forall \:x$, find the smallest possible $r$ that satisfies this equation.
...
forms a subgroup of $\mathbb{Z}^d$ under vector addition, and therefore a lattice.\footnote{\eqref{eq5.4} is a linear congruence in disguise. To see this, consider what happens when taking the logarithm of both sides.} 

\textit{Proof:} Clearly, $V \subseteq \mathbb{Z}^d$. Consider the requirements for $V$ to be a group:\\

\textbf{Closure:} $\prod_{i=1}^d b_i^{v_i+w_i} \pmod N = \prod_{i=1}^d b_i^{v_i} \cdot \prod_{i=1}^d b_i^{w_i} = 1 \cdot 1 = 1\pmod N$, hence if $b,w \in V$ then $b+w \in V$

\textbf{Inverse element:} $\prod_{i=1}^d b_i^{-v_i} = 1\pmod N$ since it is the modular inverse of the original product, which is congruent to 1 modulo N.

\textbf{Identity element:} Setting $\mathbf{v = 0}$ satisfies the relationship.

\textbf{Associativity:} Inherited by the parent group.

Therefore, the set of all vectors whose coordinates satisfy \eqref{eq5.4} forms a finite lattice $\Lambda$, and our proof is complete.
\\

 Consider that a finite n-dimensional lattice modulo N whose elements are vectors that satisfy the congruence in \eqref{eq5.4} is a subgroup of the group of all vectors with integer coordinates mod N (denoted $Z_N^n$), so we can define a coset of that lattice with respect to that group accordingly. Crucially, replacing 1 with $u$ for some arbitrary $u \in \mathbb{Z}^n_N$ defines a coset whose representative is some exponent vector $\mathbb{t}$ that represents the exact shift that must be applied to the $v_i$ to get $u$ instead of 1. This will become important later.
...
\end{equation}
where $\langle \mathbf{y,x} \rangle$ denotes the inner product operation between the vectors $\mathbf{y}$ and $\mathbf{x}$.
