...
for some $z_i \in \mathbb{Z}$. Suppose $\mathbf{z} = (z_1,z_2,\dots,z_d)$ is a vector in $\mathbb{Z}^d$. The set of all vectors $\mathbf{z}$ that satisfy the modular multiplicative congruence $X^2 \equiv 1 \pmod N$ forms a lattice which we will call $\mathit \Lambda$. Furthermore, a sublattice of $\mathit \Lambda$ can be constructed by the set of all vectors $\mathbf{z}$ that satisfy the congruence $X = \pm 1 \pmod N$. We will call this sublattice $\mathit \Lambda_0$, and denote by $\Lambda / \Lambda_0$ the set of vectors that are in $\mathit \Lambda$ but not in $\mathit \Lambda_0$. 

It is easy to see that finding a vector in $\Lambda / \Lambda_0$ is equivalent to finding a non-trivial factor of $N$. This is because $N$ will then divide the product $(X-1)(X+1)$ but will not divide either of the terms, and therefore, as explained in Chapter 1, computing $\gcd(X-1,N)$ gives us the nontrivial factor. A consequence of the Chinese Remainder Theorem is that for a number with $k$ distinct prime factors, there are $2^{k}$ possible solutions to the congruence for $\mathit \Lambda$. In our case, this means that there are 4 possible solutions. The congruence for $\mathit \Lambda_0$ will obviously have only 2 solutions regardless of $k$. This means that half of our solutions are trivial! Regev’s algorithm requires the existence of a vector $\mathbf{z}$ corresponding to a non trivial solution that satisfies \footnote{For an explanation regarding this bound, go to the end of Chapter 9}. 
...
However,at the time the original paper was published, the only way to guarantee that a sufficiently small vector would exist is a result found in the following paper \cite{LamzouriLiSoundararajan2015}.

\textbf{Lemma 6.1}: A vector satisfying \eqref{eq6.2} can be guaranteed to exist if the following upper bound holds
\begin{equation}
    n(p) < (logp)^2
\end{equation}
...
The above result, however, is dependent on the unproven Generalized Riemann Hypothesis. \footnote{In fact, it is an expansion of an old classical result that states $n(p) < C(logp)^2$} Furthermore, in Regev's own words, this bound "does not seem good enough for an improved algorithmic result", meaning it only serves to convince the reader about the validity of Regev's heuristic assumption and cannot assist in formally proving it. All this means that the algorithm's correctness is not mathematically proven but, rather, the algorithm is heuristically conjectured to be working. Let us briefly explain the connection between quadratic non-residues and non-trivial factors.
...
\textbf{Fact 1}. $n(p)$ is always prime, regardless of whether p is prime or not.\footnote{If $n(p) = ab$ with $1 < a,b < n(p)$, then a and b have to be QRs, but then $n(p)$ is also a QR, so it cannot be the least QNR}\\
...
However, this places a very large upper bound on the primes, which severely affects complexity for any practical applications. More results need to be produced to guarantee practical functionality.\\

From what we said, it is implied that generally, if X is a QR, the algorithm fails. The reader might therefore wonder regarding the probability of the algorithm's success. As we established, half of the possible solutions are trivial. However, as we will find out later, the algorithm producing enough short exponent vectors -specifically ones obeying the bound $\lVert \mathbf{z} \rVert \leq 2^{O(d)}$ from earlier, benefits our process of generating non-trivial solutions. Lastly, in regards to generating non-trivial solutions, let us state the following lemma:
...
This implies that $r+4$ elements are enough to generate $G$ with probability 1/2. This means that we necessarily need to have $\geq r+4$ samples, to avoid the possibility of landing into a QR. We will use this theorem in Chapter 10 in order to improve our odds.
