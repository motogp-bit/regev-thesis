...
for some $z_i \in \mathbb{Z}$. Suppose $\mathbf{z} = (z_1,z_2,\dots,z_d)$ is a vector in $\mathbb{Z}^d$. The set of all vectors $\mathbf{z}$ satisfying the modular multiplicative congruence $X^2 \equiv 1 \pmod N$ form a lattice which we will call $\mathit \Lambda$ \footnote{The square can be transferred to the bases to construct an equation reminiscent of \textbf{Eq 5.4}}. Furthermore, a sublattice of $\mathit \Lambda$ can be constructed by the set of all vectors $\mathbf{z}$ which satisfy the congruence $X = \pm 1 \pmod n$. We will call that sublattice $\mathit \Lambda_0$, and denote by $\Lambda / \Lambda_0$ the set of vectors that are in $\mathit \Lambda$ but not $\mathit \Lambda_0$. 
