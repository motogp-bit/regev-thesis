...
and $\mu_{i,j}$ as the associated Gram-Schmidt coefficients. Then, the basis is LLL-reduced if there exists parameter $\delta \in (1/4,1]$ such that:\\
...
\end{equation}
It should be noted that an interpretation of the 2-D determinant is that it represents the area of the parallelogram formed by the basis vectors. In n dimensions,the parallelogram becomes a parallelepiped formed by the n basis vectors,and area corresponds to volume. In general, $Vol$ can be thought of as a generalization of the determinant when there aren't necessarily enough vectors to span the whole space, so the volume of the parallelepiped formed by whatever vectors exist in our $B$.

Suppose we restrict the value computer in \textbf{Eq 9.2} to being the product of only $j$ terms, with $j < d$. Then let $Vol_j(\Lambda)$ be the volume of the parallelepiped formed by the first j basis vectors.
...
It should be mentioned that LLL's complexity is approximately $O(d^5 log^3((max (\lVert v_i \rVert))$ where d is the dimension of the input vectors. For this reason, we prefer inputting shorter vectors into $LLL$. One got therefore get a sense of the steps we will be performing in Regev's algorithm by immediately thinking back to the gaussian in the previous chapter and how it increased the probability of shorter vectors.
