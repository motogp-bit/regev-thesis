...
1. Create a superposition of all $d$-dimensional vectors $e_1 \dots e_d$, and store them in $d$ quantum registers, weighted by the gaussian function described in Chapter 8.\\
2. Create a quantum register that will contain the superposition $\prod_{i=1}^d a_i^{e_i} \pmod N$, where $a_i$ are the squares of the first $d$ primes, using the procedure described in Chapter 7.\\
...
6. Repeat steps 1 through 5 $d+k$ times to acquire $d+k$ samples and use them to create a basis of $\Lambda'$, akin to the procedure described in Chapter 10.\\
7. Solve SVP on the basis of $\mathit \Lambda'$ using LLL, acquiring vectors $\mathbf{e',e'',e''',}\dots$ etc. with coordinates $e'_1 \dots e'_d, e''_1\dots e''_d$ etc. that satisfy the bound from Chapter 10.\\
