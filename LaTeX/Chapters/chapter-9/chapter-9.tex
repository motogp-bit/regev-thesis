...
To understand why we are doing this, suppose we multiply some arbitrary $u \in \mathbb{Z^d}$ with the first d columns of M. This results in a $2d+k$ dimensional vector $\mathbf{u'}$ whose first $d$ coordinates are the first $d$ coordinates of $\mathbf{u}$ and the next $d+k$ coordinates are inner products of some $\Lambda^*$ vector scaled by $\delta^{-1}$ with some $\mathbb{Z^d}$ vector, so they're really close to some integer multipled by $\delta^{-1}$. Now, suppose we define a vector $\mathbf{\hat{u}}$ that is equal to subtracting the closest integer to $\langle \mathbf{u,v_m} \rangle$ from each of the last $m$ coordinates of $u'$ (the ones that don't contain $\mathbf{u}$. Then the last $m$ coordinates of $\mathbf{\hat{u}}$ are either 0 if $\langle \mathbf{u,v_m} \rangle = 0$, or $\delta^{-1}$ if not, so 0 if $u$ is in the dual lattice of $L^*$ (e.e $L$) and $\delta^{-1}$ otherwise. 

Since $B \mathbf{\hat{u}}$ is part of the span of the vectors formed by the columns of $B$, finding the shortest vector in that span corresponds to finding the $\mathbf{\hat{u}}$ with the shortest norm. Moreover, since the last $m$ coordinates of $\mathbf{\hat{u}}$ will be nearly zero, we are just looking for the shortest $\mathbf{u}$, and the process of finding short vectors will automatically filter out vectors not in $\Lambda$ by virtue of them having a squared sum of coordinates $O(k\delta^{-2})$ larger than vectors in $\Lambda$ (essentially, we are looking for shortest vectors in $\Lambda$. This means that running LLL on $M$ will provide us with at least one vector whose first $d$ coordinates are a short vector in $\Lambda$.
