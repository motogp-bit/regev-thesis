...
Now, suppose we have our $d+k$ normalized d-dimensional samples of $\mathit \Lambda^*$ $\mathbf{v_1,v_2}\dots \mathbf{v_{d+k}}$. First, by normalized, we mean that we have divided the original samples by $D$ and now every entry is in $[0,1)$, so we've mapped every sample to a unique point on the unit torus. Assuming our samples aren't exactly accurate, there has to be some additive noise $\delta_i$ for each $\mathbf{v_i}$. This means that we are assuming that $\delta_i \mathbf{v_i}$ is a very close approximation of some vector in $\Lambda^*$. Let $\delta$ be some upper bound on the magnitude of the $\delta_i$, and define the $2d+k$ dimensional lattice $\mathit \Lambda'$ spanned by the columns of $M$, where $M$ is the following matrix (we need $k \geq 4$ from Pomerance)
\begin{equation}
M = \left[
\begin{array}{c|c}
  I_{d\times d} & 0 \\
  \hline
  \begin{matrix} \delta^{-1}\mathbf{v_1^T} \\  \vdots \\ \delta^{-1}\mathbf{v_{d+k}^T} \end{matrix} & \delta^{-1} \cdot I_{k\times k}
\end{array}
\right]    
\label{eq10.1}
\end{equation}
...
\textbf{Lemma 10.1}: For any $\mathbf{u} \in \Lambda$, there exists a vector $\mathbf{u}' \in \Lambda'$ whose first d coordinates are equal to \textbf{u} with norm $\lVert \mathbf{u'} \rVert \leq \lVert \mathbf{u} \rVert \cdot \sqrt{1+d+k}$. Moreover, for $k = 4 $ it holds that the first $d$ coordinates of every nonzero column vector $\mathbf{u'} \in \Lambda'$ with norm $$\lVert \mathbf{u'} \rVert < \frac{\delta^{-1}}{6 \cdot (4\cdot det(\Lambda))^{1/d+4}}$$ form a vector in $\Lambda$ with probability at least 1/4.

To understand why we are doing this, suppose we multiply some arbitrary $u \in \mathbb{Z^d}$ with the first d columns of M. This results in a $2d+k$ dimensional vector $\mathbf{u'}$ whose first $d$ coordinates are the first $d$ coordinates of $\mathbf{u}$ and the next $d+k$ coordinates are inner products of some $\Lambda^*$ vector scaled by $\delta^{-1}$ with some $\mathbb{Z^d}$ vector, so they're really close to some integer multiplied by $\delta^{-1}$. Now, suppose we define a vector $\mathbf{\hat{u}}$ that is equal to subtracting the closest integer to $\langle \mathbf{u,v_m} \rangle$ from each of the last $m$ coordinates of $u'$ (the ones that don't contain $\mathbf{u}$. Then the last $m$ coordinates of $\mathbf{\hat{u}}$ are either 0 if $\langle \mathbf{u,v_m} \rangle = 0$, or $\delta^{-1}$ if not, so 0 if $u$ is in the dual lattice of $L^*$ (e.e $L$) and $\delta^{-1}$ otherwise. 

Since $B \mathbf{\hat{u}}$ is part of the span of the vectors formed by the columns of $B$, finding the shortest vector in that span corresponds to finding the $\mathbf{\hat{u}}$ with the shortest norm. Moreover, since the last $m$ coordinates of $\mathbf{\hat{u}}$ will be nearly zero, we are just looking for the shortest $\mathbf{u}$, and the process of finding short vectors will automatically filter out vectors not in $\Lambda$ by virtue of them having a squared sum of coordinates $O(k\delta^{-2})$ larger than vectors in $\Lambda$ (essentially, we are looking for the shortest vectors in $\Lambda$). This means that running LLL on $M$ will provide us with at least one vector whose first $d$ coordinates correspond to a short vector in $\Lambda$.
