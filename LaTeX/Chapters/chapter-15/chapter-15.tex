...
The complexity depends on the length of the chain. According to the paper \cite{KahanamokuMeyerRagavanVaikuntanathan2024}, this amounts to $l =2D$. Given $2\sqrt{d}R \leq D \leq 4\sqrt{d}R$, and $R > \sqrt{2d}$, we get $l = O(d)$. This means that $l = O(d) = O(\sqrt{n}$. The pebbling space is $2.47logl$ pebbles, so in total $O(logn)$ pebbling space.

Using these results, we get $4logD = O(\sqrt{n})$ circuit depth with multiplications, which is optimal across each run, amounts to almost half the per-run depth of optimized Shor and can be parallelized. Qubit count is bounded up by $1.3(2n + S + O(n)log(D))$ using our earlier results, which simplifies to $O(nlogn)$ qubits. Lastly, the authors claim that across all independent runs, the total number of multiplications executed is approximately half an order of magnitude better than optimized Regev circuits. Results from \cite{KahanamokuMeyerRagavanVaikuntanathan2024}, page 4 are pasted below. Note that the BKZ algorithm is preferred for lattice reduction by the paper's authors.\\
